...
export type UseEncryptedSurveyReturn = {
  ...
  allowResultForSelf: (optionIndex: number) => Promise<void>;
  allowAllResultsForSelf: () => Promise<void>;
...
export function useEncryptedSurvey({
  ...
  const allowResultFor = useCallback(
    async (grantee: `0x${string}`, optionIndex: number) => {
      ...
      try {
        ...
        const errorMessage = (error as Error).message ?? "Unable to grant access.";
        // Check if error is due to uninitialized encrypted data
        if (errorMessage.includes("reverted") || errorMessage.includes("execution reverted")) {
          setStatusMessage("Cannot grant access: survey data not initialized. Please wait for at least one vote to be submitted, or use 'Get Access' button to grant yourself access.");
        } else {
  ...
  const allowResultForSelf = useCallback(
    async (optionIndex: number) => {
      ...
      setIsSubmitting(true);
      setStatusMessage("Granting yourself decryption access...");
      try {
        const tx = await writeContract.allowResultForSelf(optionIndex);
        await tx.wait();
        setStatusMessage("Decryption access granted to yourself.");
      } catch (error) {
        const errorMessage = (error as Error).message ?? "Unable to grant access.";
        if (errorMessage.includes("reverted") || errorMessage.includes("execution reverted")) {
          setStatusMessage("Cannot grant access: this option has no votes yet. Please wait for at least one vote to be submitted for this option.");
        } else {
          setStatusMessage(errorMessage);
        }
        throw error;
  ...
  const allowAllResultsForSelf = useCallback(async () => {
    if (!writeContract) {
      throw new Error("Please connect your wallet on the correct network.");
    }

    setIsSubmitting(true);
    setStatusMessage("Granting yourself access to all results...");
    try {
      const tx = await writeContract.allowAllResultsForSelf();
      await tx.wait();
      setStatusMessage("Decryption access granted to all results.");
    } catch (error) {
      const errorMessage = (error as Error).message ?? "Unable to grant access.";
      if (errorMessage.includes("reverted") || errorMessage.includes("execution reverted")) {
        setStatusMessage("Cannot grant access: no votes submitted yet. Please wait for at least one vote to be submitted first.");
      } else {
        setStatusMessage(errorMessage);
      }
      throw error;
  ...
  const decryptOption = useCallback(
    async (optionIndex: number) => {
      ...
      try {
        ...
        let friendlyMessage = "Unable to decrypt. Please ensure you have access rights and try again.";
        
        // Provide more specific but user-friendly error messages
        if (errorMessage.includes("FHE") || errorMessage.includes("encrypt") || errorMessage.includes("decrypt")) {
          friendlyMessage = "Decryption failed. Please check your access rights and try again.";
        } else if (errorMessage.includes("access") || errorMessage.includes("permission")) {
          friendlyMessage = "You don't have access to decrypt this result. Please get access first.";
